    ...
    public class BandCode
    {
        public class Code128
        {
            ...
            private void GetViewText(Bitmap p_Bitmap, string p_ViewText)
            {
                ...
                SizeF blankSize = _Graphics.MeasureString(" ", m_ValueFont);
                if (_DrawSize.Height > p_Bitmap.Height - 10 || _DrawSize.Width > p_Bitmap.Width)
                {
                    _Graphics.Dispose();
                    return;
                }
                int fillBlankNum = Convert.ToInt32((p_Bitmap.Width - _DrawSize.Width) / blankSize.Width);
                String newViewText = "";
                for (int i = 0; i < fillBlankNum-1&&i<p_ViewText.Length; i++)
                {
                    newViewText = newViewText + p_ViewText[i] + " ";
                }
                ...
                _Graphics.DrawString(newViewText, m_ValueFont, Brushes.Black, 0, _StarY);
            }
